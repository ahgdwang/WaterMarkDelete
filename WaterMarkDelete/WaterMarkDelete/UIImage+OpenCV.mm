...
- (id)WaterMarkDelete:(CGRect) rect{
    ...
    cv::Rect roiRect = cv::Rect(cv::Rect(rect.origin.x,rect.origin.y,rect.size.width,rect.size.height));
    //制作掩膜
    ...
    //图像修复
    cv::inpaint(imageMat3chan, imageMask, imageMat3chan, 9, cv::INPAINT_TELEA);
